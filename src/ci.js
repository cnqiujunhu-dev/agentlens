...
function failedResults(summary) {
...
export function formatCiPrComment(summary, { title = "AgentLens CI", artifactUrl = undefined, sarifUrl = undefined } = {}) {
  ...
  if (artifactUrl || sarifUrl) {
    lines.push("**Artifacts:**");
    if (artifactUrl) lines.push(`- Run bundle: ${artifactUrl}`);
    if (sarifUrl) lines.push(`- SARIF: ${sarifUrl}`);
    lines.push("");
  }

  if (summary.total === 0) {
    lines.push("No trace files were found. Check the `runs` input or trace generation step.");
    return lines.join("\n");
  }

  if (failed.length === 0) {
    lines.push("All recorded agent traces passed evals and enabled scan gates.");
  } else {
    lines.push(`Found ${failed.length} failing trace${failed.length === 1 ? "" : "s"}.`);
    lines.push("");
    lines.push("| Trace | Failures |");
    lines.push("| --- | --- |");
  ...
  lines.push("");
  lines.push("<details>");
  lines.push("<summary>All trace results</summary>");
  lines.push("");
  lines.push("| Trace | Status | Failures |");
  lines.push("| --- | --- | --- |");
  for (const result of summary.results) {
  ...
  lines.push("</details>");
  lines.push("");
  lines.push("_Generated by AgentLens._");
